...
void main() {
  const runIntegration = bool.fromEnvironment('RUN_SUPABASE_INTEGRATION_TESTS');
  ...
  final skipReason =
      ...
      ? false
      : 'Supabase integration tests are opt-in. Set RUN_SUPABASE_INTEGRATION_TESTS=true plus SUPABASE_URL, SUPABASE_ANON_KEY, TEST_EMAIL, and TEST_PASSWORD.';

  late SupabaseClient client;

  group('Supabase integration', skip: skipReason, () {
    setUpAll(() async {
    ...
    test(
      'insertPet round-trip: inserts and appears in fetchPets, then cleans up',
      () async {
        final db = PawTrackDatabase(client);
